    ...
    def cost_matrix(s: np.ndarray, t: np.ndarray) -> np.ndarray:
        ...
        for j in range(t.shape[0]):
            for i in range(s.shape[0]):
                d[i + 1, j + 1] = (
                    min(d[i, j + 1], d[i + 1, j], d[i, j]) + np.bitwise_xor(s[i], t[j]).sum()
                )
        return d
...
def score_similarity(
    ...
    def compare_sets(a_set, b_set):
        ...
        def get_beams(note_obj: music21.note.Note):
            return "_".join(
                ["-".join([b.type, b.direction]) if b.direction else b.type for b in note_obj.beams]
            )
        ...
        for obj in a:
            if isinstance(obj[1], music21.note.Note):
                for bObj in b:
                    if isinstance(bObj[1], music21.note.Note) and pitchComp(
                        bObj[1].pitch, obj[1].pitch
                    ):
                        if bObj[0] != obj[0]:
    ...
    def update_f1(d, key):
        prec = (
            d[key + "TP"] / (d[key + "TP"] + d[key + "FP"])
            if d[key + "TP"] + d[key + "FP"]
            else None
        )
        rec = (
            d[key + "TP"] / (d[key + "TP"] + d[key + "FN"])
            if d[key + "TP"] + d[key + "FN"]
            else None
        )
...
def test_score_similarity_same():
    score_path = "score_transformer/tokenization_tools/tokenizer/sample/input_score.musicxml"
    expected = {
        "Clef": 0,
        "KeySignature": 0,
        "TimeSignature": 0,
...
def test_score_similarity_same_2():
    ...
    res = score_similarity(tokens_to_score(" ".join(score_to_tokens(score_path))), score_path)
    # assert res == expected


def test_score_similarity_different():
    res = score_similarity(
        "/rhome/beyer/thesis/data/asap-dataset-master/Bach/Fugue/bwv_848/xml_score.musicxml",
